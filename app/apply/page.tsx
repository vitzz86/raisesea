...
export default function ApplyPage() {
  ...
  const [form, setForm] = useState({
    company_name: '', country: 'Indonesia', stage: 'Seed',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        if (k === 'raise_display' || k === 'revenue_display') return
  ...
  return (
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{
          ...
          --border:#d6e4d9;--bg:#fafcfa;--white:#fff;--red:#dc2626;
        ...
        /* LEFT – brand panel */
        .left{
          background:var(--brand);padding:52px 56px;
          display:flex;flex-direction:column;
          position:sticky;top:0;height:100vh;overflow:hidden
        }
        .left-brand{font-family:var(--font-display);font-size:22px;color:white;letter-spacing:-.3px;margin-bottom:auto}
        .left-body{flex:1;display:flex;flex-direction:column;justify-content:center;padding:40px 0}
        .left-hl{font-family:var(--font-display);font-size:52px;line-height:1.1;color:white;margin-bottom:24px}
        .left-hl em{font-style:italic;color:#a8d5b5}
        .left-sub{font-size:16px;color:#a8d5b5;line-height:1.75;font-weight:300;max-width:400px}
        .stats-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:auto}
        .stat{background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.14);border-radius:14px;padding:20px 22px}
        .stat-n{font-family:var(--font-display);font-size:34px;color:white;line-height:1}
        .stat-l{font-size:12px;color:#a8d5b5;margin-top:5px;font-weight:400}

        /* RIGHT – form panel */
        .right{background:var(--bg);padding:52px 56px;overflow-y:auto;display:flex;flex-direction:column}
      ...
      <div className="page">
        {/* ── LEFT ── */}
        <div className="left">
          <div className="left-brand">RaiseSEA</div>
          <div className="left-body">
            <h1 className="left-hl">Your pitch.<br /><em>Intelligently</em><br />matched.</h1>
            <p className="left-sub">Upload your deck and get AI-powered feedback, market benchmarks, competitive analysis, and matched to 750+ SEA investors — in 60 seconds.</p>
          </div>
          <div className="stats-grid">
            {[['750+','Investors matched'],['8','Deck dimensions scored'],['60s','Full analysis time'],['98%','Cost saving vs alternatives']].map(([n,l])=>(
              <div className="stat" key={n}><div className="stat-n">{n}</div><div className="stat-l">{l}</div></div>
            ))}
          </div>
        </div>
        ...
        <div className="right">
          {loading ? (
            <div className="proc">
              ...
              <div>
                <div className="proc-title">Analyzing your deck</div>
                <div className="proc-sub">Gemini AI is reading your pitch deck and running the full intelligence report</div>
              </div>
