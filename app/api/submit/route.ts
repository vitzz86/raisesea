...
import { deleteDeck, downloadDeck } from '@/lib/storage'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const expectedStoragePath = `${sessionUser.id}/${uniqueSlug}.pdf`
    if (storagePath !== expectedStoragePath) {
      return NextResponse.json(
        { error: 'Storage path does not belong to the signed-in user' },
        { status: 400 }
      )
    }
    ...
    if (sessionUser && !bypassFreeLimits) {
      ...
      if (clientDeckSha256) {
        const { existing, columnAvailable } = await findExistingDeckAnalysis(sessionUser.id, clientDeckSha256)
    ...
    // ── 8. Warm intro network: ALL co-investors of current investors ──
    ...
    if (submissionResult.error && isMissingDeckHashColumn(submissionResult.error)) {
      delete submissionPayload.deck_sha256
