    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-3">
          <h3 className="text-sm font-semibold text-text-primary">Moat score</h3>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5">
          ...
          {(moat.actions?.length ?? 0) > 0 && (
            <div className="border-t border-gray-100 pt-4">
              <p className="text-xs font-semibold text-text-tertiary mb-2">Actions to improve moat</p>
      ...
      {a.positioning && (
        <div>
          <h3 className="text-sm font-semibold text-text-primary mb-3">Competitive positioning</h3>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <div className="relative w-full pt-7 pb-7 px-12">
              <div className="relative w-full h-80 bg-white rounded-lg border border-gray-200">
                {/* Y axis labels */}
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs text-text-secondary font-medium whitespace-nowrap max-w-full px-2">
                  <span className="font-mono text-[10px] text-text-disabled mr-1">Y+</span>
                  ↑ {truncate(a.positioning.y_axis_label, 50)}
                </div>
                <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs text-text-tertiary font-medium whitespace-nowrap max-w-full px-2">
                  <span className="font-mono text-[10px] text-text-disabled mr-1">Y−</span>
                ...
                <div className="absolute top-1/2 -translate-y-1/2 -left-12 w-11 text-right text-xs text-text-tertiary font-medium leading-tight">
                  <div className="font-mono text-[10px] text-text-disabled">X−</div>
                  <div>{truncate(a.positioning.x_axis_label_low || `Less ${a.positioning.x_axis_label}`, 22)} ←</div>
                </div>
                <div className="absolute top-1/2 -translate-y-1/2 -right-12 w-11 text-left text-xs text-text-secondary font-medium leading-tight">
                  <div className="font-mono text-[10px] text-text-disabled">X+</div>
                  ...
                  {(a.positioning.positions || []).slice(0, 9).map((pos, i) => {
                    ...
                    return (
                      ...
                      >
                        {pos.is_founder ? (
                          <>
                            <div className="w-4 h-4 rounded-full bg-brand border-2 border-white shadow-md ring-2 ring-[#1a4d2e]/30" />
                            <span className="text-[11px] mt-1 px-1.5 py-0.5 rounded font-semibold bg-brand text-white whitespace-nowrap">{pos.name}</span>
                          </>
                        ) : (
                          <button
                            type="button"
                            ...
                            className="w-6 h-6 rounded-full bg-white border-2 border-gray-400 hover:border-brand hover:text-brand flex items-center justify-center text-[10px] font-semibold text-text-tertiary transition-colors cursor-pointer"
                          >
                            {number}
                          </button>
                        )}
            ...
            <div className="mt-5 pt-4 border-t border-gray-100">
              <p className="text-xs font-semibold text-text-secondary mb-2">Legend <span className="font-normal text-text-disabled">(click any to view details below)</span></p>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-2 text-xs">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full bg-brand border-2 border-white ring-2 ring-[#1a4d2e]/30 flex-shrink-0" />
                  <span className="text-brand font-medium">You</span>
                </div>
                {(a.positioning.positions || []).filter(p => !p.is_founder).slice(0, 8).map((pos, i) => (
                  <button
                    ...
                    type="button"
                    onClick={() => scrollToCompetitor(pos.name)}
                    className="flex items-center gap-2 text-left hover:text-brand transition-colors min-w-0"
                  >
                    <div className="w-5 h-5 rounded-full bg-white border-2 border-gray-400 flex items-center justify-center text-[9px] font-semibold text-text-tertiary flex-shrink-0">{i + 1}</div>
                    <span className="text-text-secondary truncate">{pos.name}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      {a.key_differentiators?.length > 0 && (
        <div className="bg-success-bg border border-success-border rounded-xl p-5">
          <p className="text-xs font-semibold text-success-text mb-2">Your key differentiators — state these explicitly in your pitch</p>
          <ol className="space-y-1.5">
            {a.key_differentiators.map((d, i) => (
              <li key={i} className="text-sm text-success-text flex gap-2">
      ...
      {a.white_space && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs font-semibold text-text-tertiary mb-1.5">White space opportunity</p>
          <p className="text-sm text-text-secondary leading-relaxed">{a.white_space}</p>
        </div>
      )}

      {/* Conflict warning */}
      {a.conflict_warning && (
        <div className="bg-warning-bg border border-amber-300 rounded-xl p-4">
          <p className="text-xs font-semibold text-warning-text mb-1">Investor conflict of interest</p>
          <p className="text-sm text-warning-text">{a.conflict_warning}</p>
        </div>
      )}
    </div>
  )
    ...
    <div id={competitorId(c.name)} className="bg-white border border-gray-200 rounded-xl p-4 scroll-mt-24 transition-all">
      <div className="flex items-start justify-between mb-2">
        <div>
              ...
              <span className="flex items-center gap-1.5">
                {hasWebsite && (
                  <a href={c.website} target="_blank" rel="noopener noreferrer"
                     className="text-xs text-text-tertiary hover:text-brand underline-offset-2 hover:underline">
                    ↗ Website
                  </a>
                )}
                {hasLinkedin && (
                  <a href={c.linkedin} target="_blank" rel="noopener noreferrer"
                     className="text-xs text-text-tertiary hover:text-[#0a66c2] underline-offset-2 hover:underline">
                    ↗ LinkedIn
                  </a>
                )}
              </span>
          ...
          <p className="text-xs text-text-disabled mt-0.5">{c.hq} · {c.stage} · {c.founded_year ? `Founded ${c.founded_year}` : ''}</p>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-sm font-semibold text-brand">
            {c.total_raised_usd ? `$${fmtM(c.total_raised_usd)} raised` : 'Undisclosed'}
          </p>
          <p className="text-xs text-text-disabled">{c.similarity_pct}% overlap</p>
        </div>
      </div>
      <p className="text-xs text-text-tertiary mb-3">{c.one_liner}</p>
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-success-bg rounded-lg p-2.5">
          <p className="text-xs font-medium text-success-text mb-0.5">Their strength</p>
          <p className="text-xs text-success-text">{c.key_strength}</p>
        </div>
        <div className="bg-danger-bg rounded-lg p-2.5">
          <p className="text-xs font-medium text-danger-text mb-0.5">Their weakness (your opportunity)</p>
          <p className="text-xs text-danger-text">{c.key_weakness}</p>
        </div>
      </div>
      {c.investors?.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {c.investors.slice(0,4).map((inv: string, i: number) => (
            <span key={i} className="text-xs bg-surface-muted text-text-tertiary px-1.5 py-0.5 rounded">{inv}</span>
          ))}
        </div>
      )}
    </div>
...
const moatBadge  = (s: number) => s >= 7 ? 'bg-success-bg text-success-text' : s >= 5 ? 'bg-yellow-50 text-yellow-700' : 'bg-danger-bg text-danger-text'
const fmtM       = (n: number | null) => !n ? '—' : n >= 1e9 ? `${(n/1e9).toFixed(1)}B` : n >= 1e6 ? `${(n/1e6).toFixed(1)}M` : `${(n/1e3).toFixed(0)}K`

function EmptyState() {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-10 text-center">
      <p className="text-text-tertiary text-sm font-medium mb-2">Competitive analysis not available</p>
      <p className="text-text-disabled text-xs leading-relaxed max-w-md mx-auto">
